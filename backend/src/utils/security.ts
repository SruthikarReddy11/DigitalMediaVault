...
export function createSignedHiddenNotesToken(userId: string, secret: string, expiresInMinutes = 60): string {
  const expiresAt = Date.now() + expiresInMinutes * 60 * 1000;
  const payload = Buffer.from(JSON.stringify({ userId, scope: 'hidden_notes', expiresAt })).toString('base64url');
  const signature = crypto.createHmac('sha256', secret).update(payload).digest('base64url');
  return `${payload}.${signature}`;
}

export function verifySignedHiddenNotesToken(token: string, secret: string): { userId: string } | null {
  try {
    const parts = token.split('.');
    if (parts.length !== 2) return null;
    const [payload, signature] = parts;
    ...
    if (signature !== expectedSig) return null;
    const data = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8'));
    if (!data.userId || data.scope !== 'hidden_notes' || !data.expiresAt) return null;
    if (Date.now() > data.expiresAt) return null;
    return { userId: data.userId };
  } catch {
    return null;
