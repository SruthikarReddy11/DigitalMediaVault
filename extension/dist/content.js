"use strict";(()=>{var $="vaultxmedia-floating-save-btn",q="vaultxmedia-floating-yt-btn",A="vaultxmedia-floating-maps-btn",V="vaultxmedia-save-toast",D="vaultxmedia-secret-modal",F="vaultxmedia-place-modal";function j(){let e=window.location.href.toLowerCase(),t=window.location.hostname.toLowerCase();return t.includes("youtube.com")||t.includes("youtu.be")||t.includes("vimeo.com")||t.includes("dailymotion.com")?!1:t.includes("amazon.")?e.includes("/dp/")||e.includes("/gp/product/")||e.includes("/d/"):t.includes("flipkart.com")?e.includes("/p/")||e.includes("pid="):t.includes("myntra.com")?/\/\d+\/buy/.test(e)||e.includes("/buy"):t.includes("ajio.com")?e.includes("/p/"):t.includes("meesho.com")?e.includes("/s/p/")||e.includes("/p/"):t.includes("nykaa.com")?e.includes("/p/"):t.includes("tatacliq.com")?e.includes("/p-"):t.includes("croma.com")||t.includes("reliancedigital.in")?e.includes("/p/"):!1}function z(){let e=window.location.hostname.toLowerCase(),t=window.location.href.toLowerCase();return e.includes("youtube.com")||e.includes("youtu.be")?t.includes("/watch")||t.includes("/shorts/")||t.includes("/live/")||t.includes("/embed/")||t.includes("/clip/")||e.includes("youtu.be"):!1}function R(){let e=window.location.hostname.toLowerCase(),t=window.location.href.toLowerCase();return!!(e.includes("maps.google.")||e.includes("maps.app.goo.gl")||e.includes("google.")&&(t.includes("/maps")||t.includes("/place/"))||e.includes("goo.gl")&&t.includes("/maps"))}function C(e){let t=document.getElementById(V);t&&t.remove();let o=document.createElement("div");o.id=V,o.className=`vaultx-toast ${e.isError?"vaultx-toast-error":"vaultx-toast-success"}`;let c=e.price!==void 0&&e.price!==null?`${e.currencySymbol||"\u20B9"}${e.price.toLocaleString("en-IN")}`:"",u=e.imageUrl?`<img src="${e.imageUrl}" class="vaultx-toast-img" alt="Item" onerror="this.style.display='none'" />`:`<div class="vaultx-toast-icon-box">${e.isError?"\u2715":"\u2713"}</div>`;o.innerHTML=`
  ...
  `,o.querySelector(".vaultx-toast-close")?.addEventListener("click",()=>{o.classList.add("vaultx-toast-fadeout"),setTimeout(()=>o.remove(),300)}),document.body.appendChild(o),setTimeout(()=>{o.parentElement&&(o.classList.add("vaultx-toast-fadeout"),setTimeout(()=>o.remove(),300))},5e3)}var k=null;function U(e,t){k=t||k,e.classList.remove("vaultx-loading","vaultx-error","vaultx-saved"),e.classList.add("vaultx-already-saved"),e.setAttribute("title","This product is already in your VaultXMedia Wishlist (Click to view)");let o=e.querySelector(".vaultx-btn-icon"),c=e.querySelector(".vaultx-btn-text");o&&(o.innerHTML=`
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
        <polyline points="20 6 9 17 4 12"></polyline>
      </svg>
    `),c&&(c.textContent="Already in Wishlist")}async function O(e){try{let t=await chrome.runtime.sendMessage({action:"CHECK_PRODUCT_EXISTS",url:window.location.href});t&&t.exists&&U(e,t.product)}catch{}}function G(){let e=window.location.href,t=window.location.hostname.toLowerCase(),o="",c="",u,d,p="",y="General",s=!0,x="",r="INR",M="\u20B9";if(t.includes("amazon."))x="Amazon";else if(t.includes("flipkart.com"))x="Flipkart";else if(t.includes("myntra.com"))x="Myntra";else if(t.includes("ajio.com"))x="Ajio";else if(t.includes("meesho.com"))x="Meesho";else if(t.includes("nykaa.com"))x="Nykaa";else if(t.includes("tatacliq.com"))x="Tata CLiQ";else if(t.includes("croma.com"))x="Croma";else if(t.includes("reliancedigital.in"))x="Reliance Digital";else{let v=t.replace(/^www\./,"").split(".")[0];x=v.charAt(0).toUpperCase()+v.slice(1)||"Other"}let L=v=>{if(typeof v=="number"&&!isNaN(v))return v;if(!v||typeof v!="string")return;let l=v.replace(/[^\d.]/g,"").trim(),n=parseFloat(l);return isNaN(n)||n<=0?void 0:n};if(document.querySelectorAll('script[type="application/ld+json"]').forEach(v=>{try{let l=v.textContent;if(!l)return;let n=JSON.parse(l),i=Array.isArray(n)?n:n["@graph"]?n["@graph"]:[n];for(let a of i)if(a["@type"]==="Product"||a["@type"]==="IndividualProduct"||a["@type"]==="ProductModel"){if(!o&&a.name&&(o=String(a.name).trim()),c||(typeof a.brand=="string"?c=a.brand.trim():a.brand?.name&&(c=String(a.brand.name).trim())),!p)if(typeof a.image=="string")p=a.image;else if(Array.isArray(a.image)&&a.image.length>0){let g=a.image[0];p=typeof g=="string"?g:g?.url||""}else a.image?.url&&(p=a.image.url);let m=Array.isArray(a.offers)?a.offers[0]:a.offers;if(m){if(u===void 0){let g=L(m.price||m.lowPrice);g&&(u=g)}m.priceCurrency&&(r=m.priceCurrency,M=r==="INR"?"\u20B9":r==="USD"?"$":r),m.availability&&(s=!String(m.availability).toLowerCase().includes("outofstock"))}}}catch{}}),x==="Ajio"){y="Fashion";let v=document.querySelector(".brand-name")?.textContent||document.querySelector("h2.brand-name")?.textContent||document.querySelector(".prod-brand")?.textContent;v&&v.trim()&&(c=v.trim());let l=document.querySelector(".prod-name")?.textContent||document.querySelector("h1.prod-title")?.textContent;if(l&&l.trim()&&(o=c&&!l.trim().toLowerCase().startsWith(c.toLowerCase())?`${c} ${l.trim()}`:l.trim()),u===void 0){let n=document.querySelector(".prod-sp")?.textContent||document.querySelector(".price-value")?.textContent,i=L(n);i&&(u=i)}if(d===void 0){let n=document.querySelector(".prod-cp")?.textContent||document.querySelector(".original-price")?.textContent,i=L(n);i&&(d=i)}if(!p){let n=Array.from(document.querySelectorAll("img")).filter(i=>{let a=i.getAttribute("src")||"";return a.includes("assets.ajio.com")&&(a.includes("medias")||a.includes("root")||a.includes("images"))});n.length>0&&(p=n[0].src||n[0].getAttribute("src")||"")}}else if(x==="Myntra"){y="Fashion";let v=document.querySelector(".pdp-title")?.textContent;v&&v.trim()&&(c=v.trim());let l=document.querySelector(".pdp-name")?.textContent;if(l&&l.trim()&&(o=c&&!l.trim().toLowerCase().startsWith(c.toLowerCase())?`${c} ${l.trim()}`:l.trim()),u===void 0){let n=L(document.querySelector(".pdp-price strong")?.textContent||document.querySelector(".pdp-price")?.textContent);n&&(u=n)}if(d===void 0){let n=L(document.querySelector(".pdp-mrp s")?.textContent||document.querySelector(".pdp-mrp")?.textContent);n&&(d=n)}if(!p){let n=document.querySelector(".image-grid-image")||document.querySelector('img[src*="assets.myntassets.com"]');n&&(p=n.src||n.getAttribute("src")||"")}}else if(x==="Flipkart"){let v=document.querySelector("span.B_NuCI")?.textContent||document.querySelector("h1.yhB1nd")?.textContent||document.querySelector("span._35KyD6")?.textContent;if(v&&v.trim()&&(o=v.trim()),u===void 0){let l=document.querySelector("div._30jeq3._16Jk6d")?.textContent||document.querySelector("div._30jeq3")?.textContent||document.querySelector("div.Nx9bqj.CxhGGd")?.textContent,n=L(l);n&&(u=n)}if(d===void 0){let l=document.querySelector("div._3I9_wc._2p6lqe")?.textContent||document.querySelector("div._3I9_wc")?.textContent||document.querySelector("div.yRaY8j.A68aAq")?.textContent,n=L(l);n&&(d=n)}if(!p){let l=document.querySelector("img._396cs4")||document.querySelector("img.DByuf4")||document.querySelector("img._2r_T1I");l&&(p=l.src||l.getAttribute("src")||"")}}else if(x==="Amazon"){let v=document.querySelector("#productTitle")?.textContent||document.querySelector("span#title")?.textContent;if(v&&v.trim()&&(o=v.trim()),u===void 0){let l=document.querySelector(".a-price .a-offscreen")?.textContent||document.querySelector("#priceblock_ourprice")?.textContent||document.querySelector("#corePrice_desktop .a-offscreen")?.textContent,n=L(l);n&&(u=n)}if(d===void 0){let l=document.querySelector(".a-text-price span.a-offscreen")?.textContent||document.querySelector("#listPrice")?.textContent,n=L(l);n&&(d=n)}if(!p){let l=document.querySelector("#landingImage")||document.querySelector("#imgBlkFront");l&&(p=l.src||l.getAttribute("data-old-hires")||l.getAttribute("src")||"")}}if(o||(o=document.querySelector('meta[property="og:title"]')?.content||document.querySelector('meta[name="twitter:title"]')?.content||document.title||""),p||(p=document.querySelector('meta[property="og:image"]')?.content||document.querySelector('meta[property="og:image:secure_url"]')?.content||document.querySelector('meta[name="twitter:image"]')?.content||""),x==="Ajio"&&(!o||o.toLowerCase().includes("access denied")))try{let l=new URL(e).pathname.match(/\/([^/]+)\/p\/([^/?#]+)/i);if(l&&l[1]){let n=l[1].split("-").filter(Boolean);if(n.length>0){let i=n.map(a=>a.charAt(0).toUpperCase()+a.slice(1));c=c||i[0],o=i.join(" ")}}}catch{}return o=o.replace(/\s*\|\s*Flipkart\.com$/i,"").replace(/\s*:\s*Buy Online at Best Price in India - Amazon\.in$/i,"").replace(/\s*:\s*Amazon\.in:.*$/i,"").replace(/\s*Buy Online at Ajio\.com$/i,"").replace(/\s*-\s*Ajio$/i,"").replace(/\s*Buy.*Online at Myntra$/i,"").replace(/\s*\|\s*Myntra$/i,"").trim(),(!o||o.toLowerCase().includes("access denied"))&&(o=`Product from ${x}`),{url:e,title:o,brand:c||void 0,store:x,category:y,price:u,originalPrice:d,currency:r,currencySymbol:M,imageUrl:p||void 0,inStock:s}}function Y(){let e=document.getElementById($);if(e){O(e);return}j()&&(e=document.createElement("button"),e.id=$,e.className="vaultx-floating-btn",e.setAttribute("type","button"),e.setAttribute("title","Save product directly to your VaultXMedia Wishlist"),e.innerHTML=`
  ...
  `,O(e),e.addEventListener("click",async t=>{if(t.preventDefault(),t.stopPropagation(),e.classList.contains("vaultx-loading"))return;if(e.classList.contains("vaultx-already-saved")){C({title:k?.title||"Product in Wishlist",store:k?.store,price:k?.price??void 0,currencySymbol:k?.currencySymbol,imageUrl:k?.imageUrl??void 0,message:"\u2713 This product is already in your VaultXMedia Wishlist",url:"https://digital-media-vault.vercel.app/products",isError:!1});return}e.classList.add("vaultx-loading");let o=e.querySelector(".vaultx-btn-text"),c=o.textContent;o.textContent="Saving...";let u=G();try{let d=await chrome.runtime.sendMessage({action:"SAVE_CURRENT_PRODUCT",url:window.location.href,title:u?.title||document.title,productData:u});e.classList.remove("vaultx-loading"),d&&d.success?(k=d.product,!!d.alreadyExists?(U(e,d.product),C({title:d.product?.title||"Product in Wishlist",store:d.product?.store,price:d.product?.price??void 0,currencySymbol:d.product?.currencySymbol,imageUrl:d.product?.imageUrl??void 0,message:"\u2713 This product is already in your Wishlist",url:"https://digital-media-vault.vercel.app/products",isError:!1})):(e.classList.add("vaultx-saved"),o.textContent="\u2713 Saved to Vault!",setTimeout(()=>{U(e,d.product)},2500))):(e.classList.add("vaultx-error"),o.textContent="Failed",C({title:"Failed to Save",message:d?.error||"Could not save product to VaultXMedia.",isError:!0}),setTimeout(()=>{e.classList.remove("vaultx-error"),o.textContent=c},3500))}catch(d){e.classList.remove("vaultx-loading"),e.classList.add("vaultx-error"),o.textContent="Failed",C({title:"Failed to Save",message:d.message||"Error communicating with extension worker.",isError:!0}),setTimeout(()=>{e.classList.remove("vaultx-error"),o.textContent=c},3500)}}),document.body.appendChild(e))}function K(){if(!z()){let t=document.getElementById(q);t&&t.remove();return}let e=document.getElementById(q);e||(e=document.createElement("button"),e.id=q,e.className="vaultx-floating-btn vaultx-youtube-btn",e.setAttribute("type","button"),e.setAttribute("title","Save YouTube video to Vault Theater & Videos"),e.innerHTML=`
    <span class="vaultx-btn-icon">
  ...
  `,e.addEventListener("click",t=>{t.preventDefault(),t.stopPropagation(),W(window.location.href)}),document.body.appendChild(e))}function W(e=window.location.href,t="https://digital-media-vault.vercel.app"){let o=document.getElementById(F);o&&o.remove();let c=document.createElement("div");c.id=F,c.className="vaultx-modal-backdrop";let u=document.createElement("div");u.className="vaultx-modal",u.innerHTML=`
  ...
  `,c.appendChild(u),document.body.appendChild(c);let d=()=>{c.remove(),document.removeEventListener("keydown",p)},p=r=>{r.key==="Escape"&&d()};document.addEventListener("keydown",p),u.querySelector(".vaultx-modal-close-btn")?.addEventListener("click",d),c.addEventListener("click",r=>{r.target===c&&d()});let y=u.querySelector("#vaultx-place-step-container"),s=async()=>{try{let r=await chrome.runtime.sendMessage({action:"RESOLVE_PLACE",url:e});if(!r||!r.success||!r.data){y.innerHTML=`
        ...
        `,y.querySelector("#vaultx-place-cancel")?.addEventListener("click",d),y.querySelector("#vaultx-place-retry")?.addEventListener("click",s);return}let M=r.data;x(M)}catch(r){y.innerHTML=`
      ...
      `,y.querySelector("#vaultx-place-err-close")?.addEventListener("click",d)}},x=r=>{let M=r.photoUrl?`<img src="${r.photoUrl}" class="vaultx-place-preview-img" alt="${r.name}" onerror="this.style.display='none'" />`:'<div class="vaultx-place-preview-placeholder">\u{1F4CD}</div>',L=r.rating?`<span class="vaultx-place-tag vaultx-place-tag-rating">\u2B50 ${r.rating.toFixed(1)}${r.userRatingsTotal?` (${r.userRatingsTotal.toLocaleString()})`:""}</span>`:"",H=r.category?`<span class="vaultx-place-tag">${r.category}</span>`:"";y.innerHTML=`
  ...
  `,u.appendChild(d),document.body.appendChild(u);let p=()=>{u.remove(),document.removeEventListener("keydown",y)},y=n=>{n.key==="Escape"&&p()};document.addEventListener("keydown",y),d.querySelector(".vaultx-modal-close-btn")?.addEventListener("click",p),u.addEventListener("click",n=>{n.target===u&&p()});let s=d.querySelector("#vaultx-dynamic-step-container"),x=(n,i=!0)=>{s.innerHTML=`
      <div class="vaultx-modal-error">
    ...
    `;try{let n=await chrome.runtime.sendMessage({action:"LIST_VAULT_FOLDERS"});if(!n||!n.success){n?.needAuth?r(n.error):x(n?.error||"Failed to fetch vault folders.");return}let i=n.data||[];if(i.length===0){H();return}s.innerHTML=`
        <div>
      ...
      `,s.querySelector("#vaultx-step2-cancel")?.addEventListener("click",p),s.querySelector("#vaultx-new-folder-btn")?.addEventListener("click",H),s.querySelectorAll(".vaultx-folder-item").forEach(a=>{a.addEventListener("click",()=>{let m=a.getAttribute("data-folder-id"),g=a.getAttribute("data-folder-name"),h=a.getAttribute("data-folder-color");v(m,g,h)})})}catch(n){x(n.message||"Could not load folders.")}},H=()=>{s.innerHTML=`
    ...
    `;let g=s.querySelector("#vaultx-save-cell-form"),h=s.querySelector("#vaultx-folder-pw-input"),b=s.querySelector("#vaultx-cell-title-input"),S=s.querySelector("#vaultx-cell-notes-input"),f=s.querySelector("#vaultx-save-submit"),w=s.querySelector("#vaultx-save-error-box");setTimeout(()=>{m?b?.focus():h?.focus()},80),s.querySelector("#vaultx-save-back")?.addEventListener("click",L),g.addEventListener("submit",async E=>{E.preventDefault();let _=h.value.trim(),T=b.value.trim()||t||e,X=S.value.trim()||void 0;if(!_){w.innerHTML='<div class="vaultx-modal-error">Please enter the folder password.</div>';return}f.disabled=!0,f.textContent="Verifying Password...";try{let I=await chrome.runtime.sendMessage({action:"UNLOCK_VAULT_FOLDER",folderId:n,password:_});if(!I||!I.success){f.disabled=!1,f.textContent="Unlock & Save Link",w.innerHTML=`<div class="vaultx-modal-error">${I?.error||"Incorrect folder password."}</div>`,h.focus();return}f.textContent="Saving Link...";let B=await chrome.runtime.sendMessage({action:"CREATE_VAULT_CELL",folderId:n,data:{url:e,title:T,notes:X}});B&&B.success?(p(),C({title:T,message:`\u2713 Saved in Secret Vault / ${i}`,url:`${o}/vault`,isError:!1})):(f.disabled=!1,f.textContent="Unlock & Save Link",w.innerHTML=`<div class="vaultx-modal-error">${B?.error||"Failed to save link in folder."}</div>`)}catch(I){f.disabled=!1,f.textContent="Unlock & Save Link",w.innerHTML=`<div class="vaultx-modal-error">${I.message||"Error saving link."}</div>`}})},l=async()=>{try{let n=await chrome.runtime.sendMessage({action:"CHECK_AUTH"});if(!n||!n.isAuthenticated){let a=await chrome.runtime.sendMessage({action:"SYNC_SESSION"});if(!a||!a.success){r();return}}let i=await chrome.runtime.sendMessage({action:"GET_2FA_STATUS"});if(!i||!i.success){i?.needAuth?r(i.error):x(i?.error||"Failed to connect to Secret Vault.");return}i.data?.enabled?M():L()}catch(n){x(n.message||"Failed to connect to extension.")}};l()}chrome.runtime.onMessage.addListener((e,t,o)=>{if(e.action==="EXTRACT_PAGE_PRODUCT"){try{let c=G();o({success:!0,data:c})}catch(c){o({success:!1,error:c.message})}return!0}else e.action==="SHOW_TOAST"?C(e):e.action==="OPEN_VAULT_SAVE_MODAL"?J(e.url||window.location.href,e.title||document.title,e.webUrl):e.action==="OPEN_PLACE_SAVE_MODAL"&&W(e.url||window.location.href)});function P(){if(R()){let e=document.getElementById($);e&&e.remove();let t=document.getElementById(q);t&&t.remove(),Z()}else if(z()){let e=document.getElementById($);e&&e.remove();let t=document.getElementById(A);t&&t.remove(),K()}else if(j()){let e=document.getElementById(q);e&&e.remove();let t=document.getElementById(A);t&&t.remove(),Y()}else{let e=document.getElementById($);e&&e.remove();let t=document.getElementById(q);t&&t.remove();let o=document.getElementById(A);o&&o.remove()}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",P):P();var N=location.href;new MutationObserver(()=>{let e=location.href;e!==N&&(N=e,setTimeout(P,1e3))}).observe(document,{subtree:!0,childList:!0});})();
